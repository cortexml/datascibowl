...
** NEXT [#A] Implement the classifier cost function in monitoring [2/3]
   :LOGBOOK:  
...
*** NEXT Implement theano multiclass log loss
    :LOGBOOK:  
...
def multiclass_log_loss(y_true, y_pred):
    predictions = T.clip(y_pred, 1e-15, 1 - 1e-15)
    predictions /= predictions.sum(axis=1)[:, np.newaxis]
    # row_sums is matrix size of predictions with each element
    # as that rows sums, or a column which broadcasts over rows
    # as the sum
    # row_sums = 
    predictions = predictions / row_sums

    actual = tensor.matrix(name='y_true', dtype=config.floatX)
    n_samples = actual.shape[0]
    actual[T.arange(n_samples), y_true.astype('int32')] = 1

    vectsum = actual * predictions.log()
    loss = -1.0 / n_samples * vectsum
...
** NEXT [#A] Make the workflow better
   :LOGBOOK:  
   - State "NEXT"       from ""           [2015-02-17 Tue 22:47]
   :END:      
   - use % locals() in ipynb for yaml updates?
   - a class, or an ipynb
   - yaml
   - model
   - predict
   - save predictions
** TODO [#B] Get running on GPU/desktop
   -
